...
enum MonitorColorSchemePreference: String, CaseIterable, Identifiable {
    ...
    var title: String {
        ...
        case .sunrise:
            String(localized: "color-scheme.sunrise")
        case .mono:
            String(localized: "color-scheme.mono")
        }
    }
}
...
final class MonitorSettings: ObservableObject {
    ...
    private func migrateMetrics(_ ids: [String], for kind: MonitorKind) -> [String] {
        let mapping: [String: String] = {
            switch kind {
            case .cpu:
                return ["系统": "system", "用户": "user", "闲置": "idle", "启动时间": "temperature", "温度": "temperature"]
            case .gpu:
                return ["GPU内存": "gpu-memory", "已分配": "allocated", "渲染": "render", "分块": "tiler", "温度": "temperature"]
            case .memory:
                return ["已用": "used", "压力": "pressure", "交换已用": "swap-used", "总量": "total", "App 占用": "app-memory", "缓存": "cached", "压缩": "compressed"]
            case .storage:
    ...
    func resetAll() {
        ...
        colorSchemePreference = .balanced
        ringSource = .combined
