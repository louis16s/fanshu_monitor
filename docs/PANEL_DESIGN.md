...
- 顶部品牌、版本、时钟与操作按钮使用清晰的单行布局；按钮至少有 24 pt 的点击区域
- 模块背景保持中性玻璃色，模块色用于图标、曲线、进度和状态，不整块染色
- 普通模块与 Display 使用相同的 8 pt 圆角；设置分组也使用 8 pt 圆角
- 不为视觉效果引入持续动画或额外采样，面板关闭时仍遵守原有休眠策略
